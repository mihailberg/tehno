...
if($this->InitComponentTemplate($templatePage))
{
	...
	if(strlen($folderPath) > 0)
	{
      ...
      $myResult = CIBlockElement::GetList(
        array("catalog_PRICE_7" => "asc"),
        array("ACTIVE" => "Y", "IBLOCK_ID" => 13, "SEARCHABLE_CONTENT" => "%{$arFilter["QUERY"]}%"),
        false,
        array("iNumPage" => 1, "nPageSize" => 8),
        $arSelect
      );

      while ($row = $myResult -> Fetch()) {
        $row["PREVIEW_PICTURE"] = (0 < $row["PREVIEW_PICTURE"] ? CFile::GetFileArray($row["PREVIEW_PICTURE"]) : false);
        $arResult["SEARCH"][] = $row;
      }
