...
class SaleBasketLineComponent extends CBitrixComponent
{
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		if ($arParams['POSITION_VERTICAL'] != 'bottom')
			$arParams['POSITION_VERTICAL'] = 'top';

		if ($arParams['POSITION_HORIZONTAL'] != 'left')
	...
	public function executeComponent()
	{
		...
		if (! (isset($_SESSION["SALE_BASKET_NUM_PRODUCTS"][SITE_ID]) && $_SESSION["SALE_BASKET_NUM_PRODUCTS"][SITE_ID] === 0))
		{
			...
			else
			{
				if($this->arParams["SHOW_TOTAL_PRICE"] == "Y")
					...
					$this->arResult["NUM_PRODUCTS"] = $this->getNumProducts();
			}

			if (is_array($_SESSION["SALE_BASKET_NUM_PRODUCTS"]))
				$_SESSION["SALE_BASKET_NUM_PRODUCTS"][SITE_ID] = $this->arResult["NUM_PRODUCTS"];
			else
				$_SESSION["SALE_BASKET_NUM_PRODUCTS"] = array(SITE_ID => $this->arResult["NUM_PRODUCTS"]);
		}

		if($this->arParams["SHOW_TOTAL_PRICE"] == "Y")
			$this->arResult["TOTAL_PRICE"] = CCurrencyLang::CurrencyFormat($this->arResult["TOTAL_PRICE"], CSaleLang::GetLangCurrency(SITE_ID), true);

		$productS = $this->BasketNumberWordEndings($this->arResult["NUM_PRODUCTS"]);
	...
	private function getNumProducts()
	{
		...
		{
			$numProducts = $_SESSION["SALE_BASKET_NUM_PRODUCTS"][SITE_ID];
		}
		elseif ($userFilter = $this->getUserFilter())
		{
	...
	public function BasketNumberWordEndings($num, $lang = false, $arEnds = false) // TODO refactor in sale
	{
		if ($lang===false)
			$lang = LANGUAGE_ID;
